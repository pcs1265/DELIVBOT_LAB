...
DOC_STATUS_IN_DELIVERY = 1
DOC_STATUS_ABANDONED = 2
...
def openBox():
    ...
    print(data)
    return {"당신이 요청한 번호" : data['docbox_num']}
...
@operationAPI.route('/currStoredUser', methods=["GET"])
def currStoredUser():
    # 데이터베이스 커서 생성
    cur = rosclient.database.cursor()
    
    # 데이터베이스로부터 현재 배송완료 되지않은 문서 행 가져오기     
    sql = "SELECT d.user_id, d.doc_id, d.docbox_num " \
          "FROM doc_table d " \
